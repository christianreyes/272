...
def average_time_of_arrival (times = nil)
    if(times != nil and times.class == [].class)
        valid_time_pattern = /^(\d{1,2}):(\d{1,2})(am|pm|AM|PM)/
	    ...
	    times.each do |time|	
		    ...
		    if(match_data == nil)
			    return "Please check input times"
			    arr_time = Array.new
			    break
		    else
		    	t = Time.parse(time)
	    ...
	    if(arr_time.length > 0)
		    ...
		    return (arr_time[0]+average).strftime("%I:%M%P")
	    end
    else
        return nil;
    end
